import { homePage, helperClass } from "../../pages/PageIndex";
...
describe("End to End Tests for a Workflow", () => {
  it("Step 1 : Verify that the category for Summer Dresses exists and then click on the menu", () => {
    ...
    homePage
      ...
      .click({ force: true });
    homePage
      .getHeader() //verifies the page heading
  ...
  it("Click on All specials button", () => {
    homePage
      ...
      .then(() => {
        homePage.getAllSpecials().click({ force: true });
      });
    //verifies the page heading
    helperClass.getH1PageHeader().should("contain", "Price drop");
  });
  ...
  it("Verify the first product, choose qty, size, color and adds product to cart", () => {
    ...
    cy.get("#quantity_wanted").clear().type(productOptions.quamtity);
    cy.get("select").select(productOptions.size);
    ...
    cy.get("tbody > tr > td.cart_description")
      ...
      .should(
        "have.text",
        "Color : " + productOptions.color + ", Size : " + productOptions.size
      );
